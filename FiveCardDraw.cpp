...
/**************************************************/
NullBuffer null_buffer_draw;
ostream null_stream_draw(&null_buffer_draw);

#if DEBUG
#define print_debug cout
#else
#define print_debug null_stream_draw
#endif
/**************************************************/
...
*/
int
FiveCardDraw::beforeTurn(Player &plr) {
	int flag = 0;
	char c[4];
	size_t pos = 0;
	string input;
	...
	while (!flag) {
		...
		cout << "**************************************" << endl;
		cout << "Which card(s) would you like to discard? Enter <All> to discard all cards." << endl;
		cout << "e.g. 1" << endl;
		cout << ">> ";
		cin >> input;
		// Discard all cards.
		if (input == "All") {
			size_t i = 0;
			while (i < plr.hand.sizeHand()) {
				flag = 1;
				discard_deck.addCard(plr.hand[i]);
				plr.hand.removeCard(i);
		...
		if (isdigit(c[0])) {
			...
			if (pos <= 5 && pos >= 0) {
				...
				cout << "**************************************" << endl;
				cout << "Options: <All>, <0>, <1>, <2>, <3>, <4>, <5>." << endl;
				...
				cout << "<All>: Discards all cards." << endl;
				cout << "**************************************" << endl;
				cout << "Try again!" << endl;
			}
			...
			flag = 0;
			cout << "Warning -> Invalid inputs." << endl;
			cout << "Inputs must be numbers between 0 and 5, or <All>." << endl;
			cout << "Try again!" << endl;
		}
...
/*
*	A public virtual turn method that takes a reference to a Player and returns an int.
*	The method should deal the player's hand as many cards from the main deck as the player had discarded
*	(so that the player again ends up with 5 cards in their hand).
*
*	If the deck is (or becomes) empty, the discard deck should be shuffled and any remaining replacement
*	cards should be dealt to the player's hand from the discard deck instead.
*	If there are not enough cards left in the main or discard decks to replace the player's discarded cards,
*	the method should return a non-zero value to indicate failure: otherwise the method should return 0 to indicate success.
*/
int
FiveCardDraw::Turn(Player& plr) {

	size_t num_of_card_discarded = HAND_SIZE - plr.hand.sizeHand();

	if (num_of_card_discarded == 0) {
		cout << plr.name << " already has five cards." << endl;
	}
	else {
		size_t i = 0;
		while (i < num_of_card_discarded) {
			if (main_deck.sizeDeck() > 0) {
				plr.hand << main_deck;
				++i;
			}
			else {
				if (discard_deck.sizeDeck() > 0) {
					discard_deck.shuffleDeck();
					plr.hand << discard_deck;
					++i;
				}
				else {
					return MAIN_DECK_AND_DISC_DECK_EMPTY;
				}
			}
		}
	}
...
*/
int
FiveCardDraw::afterTurn(Player & plr) {
	cout << "**************************************" << endl;
	cout << "Player's name: " << plr.name << endl;
	cout << plr.name << "'s hand: " << plr.hand << endl;
	cout << "******************************************" << endl;
...
FiveCardDraw::beforeRound() {
	...
	while (idx < HAND_SIZE) {
		size_t i = 0;
		while (i < number_of_players) { // TODO: add error checks for when we only have one player.
			size_t current_player_position = (dealer_position + i + 1) % number_of_players; // current player.
...
void 
FiveCardDraw::bettingPhase(size_t & number_of_players, bool &no_previous_bet) {
	...
	while (j_1 < number_of_players) {
		...
		cout << plrs_in_game[current_player_position]->name 
			 << "\nhand: " << plrs_in_game[current_player_position]->hand << endl;
		...
		if (str1 == "check") {
			...
			while (j_2 < (number_of_players - played)) {
				...
				cout << plrs_in_game[other_player_position]->name 
					 << "\nhand: " << plrs_in_game[other_player_position]->hand << endl;
...
FiveCardDraw::Round() {
	...
	while (i < plrs_in_game.size()) {
		...
		r_status = beforeTurn(*plrs_in_game[current_player_position]);

		if (r_status != SUCCESS) {
			return r_status;
		}
		else {
			r_status = Turn(*plrs_in_game[current_player_position]);
			...
			if (r_status != SUCCESS) {
				...
				r_status = afterTurn(*plrs_in_game[current_player_position]);
	...
	size_t num_of_players_in_phase_2 = plrs_in_game.size();
	bettingPhase(num_of_players_in_phase_2, no_previous_bet);
...
/*
*	Declare and define an overloaded poker_rank non-member function that
...
bool pokerRankPlayers(shared_ptr<Player> plr_x, shared_ptr<Player> plr_y) {
	...
	if (plr_x == nullptr) {
		...
		if (plr_y == nullptr) {
			...
			r_status = pokerRankHands(plr_x->hand, plr_y->hand);
			return r_status;
		}
	}
}
...
*/
hand_enum
FiveCardDraw::playerHandRank(const Hand & hd) {
	Hand hd_ = hd;
...
ostream &operator<< (Deck &d_x, Deck &d_y) {
	d_x.card_vector_in_deck.push_back(d_y.card_vector_in_deck.back());
	sort(d_x.card_vector_in_deck.begin(), d_x.card_vector_in_deck.end());
	d_y.card_vector_in_deck.pop_back();
	ostream &o = cout;
	//o << h.asString() << endl;
	return o;
}
...
ostream &operator<< (Deck &d, Hand &h) {
	d.card_vector_in_deck.push_back(h.card_vector_in_hand.back());
	sort(d.card_vector_in_deck.begin(), d.card_vector_in_deck.end());
	h.card_vector_in_hand.pop_back();
	ostream &o = cout;
	//o << h.asString() << endl;
	return o;
}
...
size_t FiveCardDraw::sizeOfPlayerVector() {
